...
class TestAwsIamMethods(TestCase):
    ...
    def test_auth_aws_iam(self, login_mock, datetime_mock):
        ...
        actual_iam_request_url = b64decode(actual_params["iam_request_url"]).decode(
            "utf-8"
        )
        ...
        actual_iam_request_body = b64decode(actual_params["iam_request_body"]).decode(
            "utf-8"
        )
        self.assertEqual(
            "Action=GetCallerIdentity&Version=2011-06-15", actual_iam_request_body
        )
